...
bins_ht = array.array('d', [0.5, 0.75, 1, 1.25, 1.75, 2.5, 3.5])
def build_graph(df, dataset):
    ...
    df = df.Define("n_bjets", "FCCAnalyses::ReconstructedParticle::get_n(sel_bjets)")
     # select jets
       #df = df.Define("jets", "FCCAnalyses::ReconstructedParticle::get(Jet_objIdx.index, ReconstructedParticles)")
    ...
    # calculate HT
    df = df.Define("HT", "pT_leptons_sel[0] + pT_leptons_sel[1] + pT_leptons_sel[2] + pT_leptons_sel[3] + sel_bjets_pt[0] + sumJetPt")
