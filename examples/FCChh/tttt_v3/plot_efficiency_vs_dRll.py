...
output_dir = "/eos/user/l/lberiet/www/4t_analysis/4l/SS_OF=4"

# Ensure output directory exists
os.makedirs(output_dir, exist_ok=True)

# Find all Q bin files
process_base = "mgp8_pp_tttt_wmlep_Q_*_5f_84TeV"
...
for q_bin in q_bins:
    ...
    all_file = os.path.join(input_dir, f"mgp8_pp_tttt_wmlep_Q_{q_bin}_5f_84TeV_all_events_histo.root")
    #sel0_sf_ss_file = os.path.join(input_dir, f"mgp8_pp_tttt_wmlep_Q_{q_bin}_5f_84TeV_sel0_sf_ss_histo.root")
    sel1_file = os.path.join(input_dir, f"mgp8_pp_tttt_wmlep_Q_{q_bin}_5f_84TeV_sel1_sf_ss_histo.root")
    sel2_file = os.path.join(input_dir, f"mgp8_pp_tttt_wmlep_Q_{q_bin}_5f_84TeV_sel2_bjets_histo.root")
    sel3_file = os.path.join(input_dir, f"mgp8_pp_tttt_wmlep_Q_{q_bin}_5f_84TeV_sel3_jets_histo.root")
    sel4_file = os.path.join(input_dir, f"mgp8_pp_tttt_wmlep_Q_{q_bin}_5f_84TeV_sel4_notZ_histo.root")

    # Open files
    files = [
        (all_file, "all"),
        #(sel0_sf_ss_file, "sel0_sf_ss"),
        (sel1_file, "sel1_sf_ss"),
...
if not all([h_all, h_sel1, h_sel2, h_sel3, h_sel4]):
...
eff1 = get_efficiency_hist(h_all, h_sel1, "eff_sel1_sf_ss", ROOT.kMagenta + 3)
...
eff1.GetYaxis().SetRangeUser(0, 0.05)
eff1.Draw("HIST")
eff2.Draw("HIST SAME")
eff3.Draw("HIST SAME")
eff4.Draw("HIST SAME")
...
legend.AddEntry(eff1, "sel1_sf_ss", "l")
