...
outputDir = "/eos/user/l/lberiet/ttZ_diff_results/new_iso_noORL"
...
def build_graph(df, dataset):
    ...
    # Merge electrons and muons without overlap removal
    df = df.Define("sel_leptons_unsort", "FCCAnalyses::ReconstructedParticle::merge(electrons, muons)")
    ...
    df = df.Define("clean_leptons_temp", "FCCAnalyses::ReconstructedParticle::merge(electrons_isolated, muons_isolated)")
    ...
    df = df.Define("n_leptons", "FCCAnalyses::ReconstructedParticle::get_n(clean_leptons)")
   
    # Use all jets without overlap removal
    df = df.Define("clean_jets", "Jet")
    ...
    # For b-jets, use all b-tagged jets without overlap removal
    ...
    df = df.Define("Z_ll_and_second_pairs", "AnalysisFCChh::getZllAndSecondOSPair(electrons_isolated, muons_isolated)")
