...
outputDir = "/eos/user/l/lberiet/Histmaker/ttH/"
...
def build_graph(df, dataset):
    ...
    df = df.Define('Z_ll_1_mass', 'FCCAnalyses::ReconstructedParticle::get_mass(Z_ll_and_second_pairs_merged)[0]')
    df = df.Define('Z_ll_2_mass', 'FCCAnalyses::ReconstructedParticle::get_mass(Z_ll_and_second_pairs_merged)[1]')
    df = df.Define('Z_ll_1_pt', 'FCCAnalyses::ReconstructedParticle::get_pt(Z_ll_and_second_pairs_merged)[0]')
    df = df.Define('Z_ll_2_pt', 'FCCAnalyses::ReconstructedParticle::get_pt(Z_ll_and_second_pairs_merged)[1]')
    #df = df.Define('Z_ll_2_flavor', 'Z_ll_and_second_pairs[1].flavour_flag') #1 or 2 for SF
    df = df.Define("Pair1_Pair2_mass", 'FCCAnalyses::ReconstructedParticle::get_mass(Z_ll_and_second_pairs_merged)[0] + FCCAnalyses::ReconstructedParticle::get_mass(Z_ll_and_second_pairs_merged)[1]')
    ...
    results.append(df.Histo1D(("Z_ll_1_mass_pre", "", *bins_count), "Z_ll_1_mass"))
    results.append(df.Histo1D(("Z_ll_2_mass_pre", "", *bins_count), "Z_ll_2_mass"))
    results.append(df.Histo1D(("Z_ll_1_pt_pre", "", *bins_count), "Z_ll_1_pt"))
    results.append(df.Histo1D(("Z_ll_2_pt_pre", "", *bins_count), "Z_ll_2_pt"))
    #results.append(df.Histo1D(("Z_ll_1_flavor_pre", "", *bins_count), "Z_ll_1_flavor"))
    #results.append(df.Histo1D(("Z_ll_2_flavor_pre", "", *bins_count), "Z_ll_2_flavor"))
    results.append(df.Histo1D(("Pair1_Pair2_mass_pre", "", *bins_count), "Pair1_Pair2_mass"))
    ...
    df = df.Define(f"cut{len(selections)}", f"{len(selections)}")
    results.append(df.Histo1D(("cutFlow", "", *bins_count), f"cut{len(selections)}"))
    selections.append("N_{lep} >= 1 & N_{lep} <= 2")
    ...
    df = df.Filter("Z_ll_1_mass < 80 || Z_ll_1_mass > 100")
    df = df.Define(f"cut{len(selections)}", f"{len(selections)}")
    results.append(df.Histo1D(("cutFlow", "", *bins_count), f"cut{len(selections)}"))
    selections.append("Exclude mass < 80 || mass > 100")



    # cut on Z_ll mass
    #df = df.Filter("Z_ll_2_mass < 60.")
    #df = df.Define(f"cut{len(selections)}", f"{len(selections)}")
    #results.append(df.Histo1D(("cutFlow", "", *bins_count), f"cut{len(selections)}"))
    #selections.append("Z* mass < 60.")
    ...
    bins_zpt = array.array('d', [0, 100, 200, 300, 400, 500, 600, 700, 800, 950, 1150, 1500])#, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000 ])  # GeV
    results.append(df.Histo1D(("Z_ll_1_pt_sel", "", len(bins_zpt) - 1, bins_zpt), "Z_ll_1_pt"))
    results.append(df.Histo1D(("Z_ll_1_pt_sel_eleId_wp", "", len(bins_zpt) - 1, bins_zpt), "Z_ll_1_pt", "wp_eleId"))
    results.append(df.Histo1D(("Z_ll_1_pt_sel_eleId_wm", "", len(bins_zpt) - 1, bins_zpt), "Z_ll_1_pt", "wm_eleId"))
    results.append(df.Histo1D(("Z_ll_1_pt_sel_muId_wp", "", len(bins_zpt) - 1, bins_zpt), "Z_ll_1_pt", "wp_muId"))
    results.append(df.Histo1D(("Z_ll_1_pt_sel_muId_wm", "", len(bins_zpt) - 1, bins_zpt), "Z_ll_1_pt", "wm_muId"))
    results.append(df.Histo1D(("Z_ll_1_pt_sel_bjetId_wp", "", len(bins_zpt) - 1, bins_zpt), "Z_ll_1_pt", "wp_bjetId"))
    results.append(df.Histo1D(("Z_ll_1_pt_sel_bjetId_wm", "", len(bins_zpt) - 1, bins_zpt), "Z_ll_1_pt", "wm_bjetId"))                            
